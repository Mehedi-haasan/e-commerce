  ...
  return (
    <div className='bg-[#F6F6F6]'>
        ...
        <div className='w-full md:w-[95%] lg:w-[90%] mx-auto'>
            <div className='flex'>
               <h1 className='text-2xl mx-auto font-semibold ml-4 md:ml-0 py-10'>Ladies Cornar</h1>
            </div>
        </div>
        <div className='grid grid-cols-12 mx-auto w-full md:w-[95%] lg:w-[90%] gap-2 mt-3'>
        {data.filter((item)=>item.category === "Ladies").map(({id,discount,image,colour,input,size,heading,stock,price,rule,category}) => (
                <div key={uuidv4()} className="grid col-span-12  md:col-span-4 lg:col-span-4 xl:col-span-3   duration-300 mx-auto border shadow-lg p-4 w-full px-4 rounded-lg">
                  <LadiesCard id={id}  discount={discount} image={image} input={input} colour={colour} category={category} rules={rule} size={size} heading={heading} stock={stock} price={price}/>
                </div>
              ))}
        </div>
    </div>
