...
const HotSaleCart = ({id, heading, image, input, colour, size, price, rating, stock, category, discount,rules}) => {
  ...
  const details = useNavigate();
    ...
    return (
        <div className='w-full p-8 rounded-md shadow mx-auto bg-white'>
        ...
        <h1 className='font-bold py-2 lg:pt-10 text-lg'>{heading}</h1>
        ...
        <h2 className='text-lg hover:text-red-500 font-bold'>Price: {price}$</h2>
        {
         stock ? <h3 className='mt-1 font-semibold text-lg flex'><Icon icon="icon-park-solid:correct" className='mt-1 mr-1 text-red-500'/> In Stock</h3>:<h3 className='pb-2 pt-1 font-semibold'>Out of Stock</h3>
        }
